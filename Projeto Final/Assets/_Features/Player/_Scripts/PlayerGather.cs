...
public class PlayerGather : MonoBehaviour
{
    ...
    public void Gather()
    {
        if (pm.isGathering == true) return;

        if (pm.isCarrying == true)
        {
        ...
        if (collider != null)
        {
            ...
            if (pm.isCarrying == true && currentCarryablePrefab != null) DropCarryable(currentCarryablePrefab); // Apenas por seguran�a
    ...
    private IEnumerator CarryGatherable(GameObject gatherable)
    {
        ...
        pm.isCarrying = true;
        pm.isGathering = false;
        pm.animator.SetBool("IsGathering", pm.isGathering);
        pm.animator.SetBool("IsCarrying", pm.isCarrying);
    }
